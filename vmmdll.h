...
/*
* 使用命令行参数初始化 VMM.DLL。有关参数的详细信息，
* 请参阅 GitHub 上的 MemProcFS 和 LeechCore 的 wiki。
* 注意！LeechCore 的初始化参数也适用于此函数。
* 重要参数包括：
*    -printf = 显示 printf 样式的输出。
*    -v -vv -vvv = 额外的详细级别。
*    -device = 设备格式（用于 LeechCore） - 请参阅 leechcore.h 或
*              GitHub 文档以获取更多信息。部分值包括：<file>、fpga、usb3380、
*              hvsavedstate、totalmeltdown、pmem。
*    -remote = 远程 LeechCore 实例 - 请参阅 leechcore.h 或 GitHub
*              文档以获取更多信息。
*    -norefresh = 禁用后台刷新（即使后备内存是易失性内存）。
*    -memmap = 指定物理内存映射文件或指定 'auto'。
*              例子：-memmap c:\\temp\\my_custom_memory_map.txt
*              例子：-memmap auto
*    -pagefile[0-9] = 除物理内存外还使用的页面文件。
*              通常 pagefile.sys 的索引为 0，swapfile.sys 的索引为 1。
*              页面文件在不断变化 - 如果内存转储和页面文件之间的时间差超过几分钟，请勿使用。
*              例子：'pagefile0 swapfile.sys'
*    -disable-python = 阻止加载 python 插件子系统。
*    -disable-symbolserver = 禁用符号服务器，直到用户更改。
*              此参数将优先于注册表设置。
*    -disable-symbols = 禁用从 .pdb 文件进行的符号查找。
*    -disable-infodb = 禁用 infodb 和通过它进行的任何符号查找。
*    -waitinitialize = 等待初始化完成后再返回。
*              正常情况下，一些初始化是异步进行的，可能在初始化调用完成时尚未完成。
*              这包括虚拟内存压缩、注册表等。
*              例子：'-waitinitialize'
*    -userinteract = 允许 vmm.dll 在控制台上查询用户以获取信息，
*              如 leechcore 设备选项等。
*              默认：用户交互 = 禁用。
*    -vm = 虚拟机（VM）解析。
*    -vm-basic = 虚拟机（VM）解析（仅限物理内存）。
*    -vm-nested = 虚拟机（VM）解析（包括嵌套的 VM）。
*    -forensic-yara-rules = 使用指定规则执行法医 yara 扫描。
*              应指定源或已编译的 yara 规则的完整路径。
*              例子：-forensic-yara-rules "C:\Temp\my_yara_rules.yar"
*    -forensic = 启动时立即开始物理内存的法医扫描（如果可能）。
*              允许的参数值范围为 0-4。
*              注意！法医模式不适用于实时内存。
*              1 = 带内存 sqlite 数据库的法医模式。
*              2 = 带临时 sqlite 数据库的法医模式，退出时删除。
*              3 = 带临时 sqlite 数据库的法医模式，退出时保留。
*              4 = 带静态命名 sqlite 数据库（vmm.sqlite3）的法医模式。
*              例子 -forensic 4
*
* -- argc: 参数数量
* -- argv: 参数数组
* -- ppLcErrorInfo: 可选指针，用于在失败时接收由函数分配的
*              LC_CONFIG_ERRORINFO 结构的内存，包含扩展错误信息。
*              调用 LcMemFree() 释放任何接收到的内存。
* -- 返回值: 成功返回 VMM_HANDLE，用于后续的 API 调用。失败返回 NULL。
*/
 ...
 * 函数：VMMDLL_Close
 * 描述：
 *  此函数用于关闭已实例化的VMM句柄并释放相关联的资源。
 *  调用本函数后，句柄hVMM不再有效，不应继续使用。
 * 参数：
 *  hVMM - 用于虚拟机管理组件的句柄。此句柄通过调用VMMDLL_Open或其他相关函数获得。
 *          它用于标识和访问虚拟机管理组件的具体实例。
 * 返回值：
 *  无
 * 备注：
 *  本函数是虚拟机管理组件API的一部分，对外导出供使用。
 *  通常在客户端完成对虚拟机管理组件的使用或需要释放资源以避免内存泄漏时调用。
 */
EXPORTED_FUNCTION
...
/*
 * 函数：VMMDLL_CloseAll
 * 描述：此函数负责关闭所有已实例化的VMM_HANDLE版本并释放所有相关资源。
 * 它是VMMDLL库的一个导出函数，旨在被外部应用程序或模块调用以执行清理操作。
 * 参数：无
 * 返回值：无
 */
/*
* 关闭所有已实例化的VMM_HANDLE并释放所有资源。
*/
EXPORTED_FUNCTION
...
/*
* 获取设备特定的选项值。请参阅定义 VMMDLL_OPT_* 以获取有关有效选项值的
* 信息。请注意，不同设备类型的选项值可能会重叠，但含义不同。
* -- hVMM: VMM 句柄
* -- fOption: 要获取的选项值
* -- pqwValue: 指向接收选项值的 ULONG64 类型指针
* -- return: 成功/失败
*/
...
/*
* 设置设备特定的选项值。请参阅定义 VMMDLL_OPT_* 以获取有关有效选项值的
* 信息。请注意，不同设备类型的选项值可能会重叠，但含义不同。
* -- hVMM: VMM 句柄
* -- fOption: 要设置的选项值
* -- qwValue: 要设置的选项值
...
/*
* VMM_VFS_FILELIST2 结构中回调函数的辅助函数。
*/
EXPORTED_FUNCTION
...
/*
* 在 MemProcFS 中列出一个目录中的文件。通过在 pFileList 参数中提供的回调函数来列出
* 目录和文件。
* 如果需要单个文件的信息，则需要列出其所在目录中的所有文件。
* -- hVMM: VMM 句柄
* -- [uw]szPath: 要列出文件的路径
* -- pFileList: 文件列表结构指针
* -- return: 成功/失败
