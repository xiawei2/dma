...
int64_t MapData::GetTraversalPtr(int64_t ptr, int64_t offset, int t) {
    int64_t result = 0;
    if (t == 1) {
...
vector<Coordinate> MapData::GetMonsterData() {
...
vector<Coordinate> MapData::GetItemData() {
    std:
...
Coordinate MapData::GetPosition(long long int ptr) {
...
Coordinate MapData::GetNearestPosition(Coordinate position, vector<Coordinate> positions) {
    int minIndex = 0;
...
int MapData::ComputedRange(Coordinate position, Coordinate position1) {
    // 计算x坐标的差值
 ...
 * @param direction
 */
void MapData::passMapByRun(int direction) {
    // 人物指针
    auto personPtr = GetPersonPtr();
    // 地图偏移
    long mapPy = mem.readLong(personPtr + 地图偏移);
    if (mapPy == 0) {
        return;
    }
    long roomStructure = mem.readLong(mem.readLong(mem.readLong(房间编号) + 时间基址) + 顺图偏移);
    long positonStructure = roomStructure + direction * 方向ID + 坐标顺图;
...
//    move(x, y);
//    Timer.sleep(50);
//    CoordinateType nowRomm = GetCutRoom();
//    if (nowRomm.x == cutRoom.x && nowRomm.y == cutRoom.y) {
